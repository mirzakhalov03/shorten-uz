...
export const isPasswordValid = async (password: string, userPasswordHash: string) => 
    await bcrypt.compare(password, userPasswordHash);

export const validatePasswordOrSendError = async ( res: Response, password: string, userPasswordHash: string, userId: number ) => {
...
export const verifyRefreshTokenOrSendError = (res: Response, refreshToken: string): { userId: number } | null => {
    try {
        return jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET!) as { userId: number };
    } catch {
